...
function mapDict<T1, T2>(items: Dict<T1>, func:(item: T1, k: string) => T2):Dict<T2> {
  let newDict: Dict<T2> = {}
  Object.entries(items).forEach((ele:[string, T1]) => {
    let newEle:T2 = func(ele[1], ele[0])
    newDict[ele[0]] = newEle
  })
  console.log(newDict)
  return newDict
}
// Array.prototype.filter, but for Dict
function filterDict<T1>(items: Dict<T1>, func: (item: T1) => boolean): Dict<T1> {
  let newDict: Dict<T1> = {}
  let filteredItem: T1[] = Array.prototype.filter.bind(Object.values(items), func)
...
function reduceDict<T1, T2>(items: Dict<T1>, func: (acc: T2, item: T1) => T2, init: T2): T2 {
  let value:T2 = init
  Object.entries(items).forEach((ele: [string, T1]) => {
    value = func(value, ele[1])
  })
  return value
}
